...
// Authserver
//
// This server will store user account information, including
// a uuid and a user name. If the user is not found, it still
// returns a result, with standard "missing" data fields
//
...
func main() {
	...
	flag.Parse()
	logFileName := fmt.Sprintf("%s/etc/%s.xml", fullpath, *logFile)
