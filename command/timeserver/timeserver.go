...
// Timeserver
//
// This server will show the time in the current timezone in
// Hours, Minutes, and Seconds on the "/time" url.
// It also allows for a user to log in to create a personalized
// message when displaying the time as well as a greeting on the
// homepage. All other urls will return a 404.
//
// Building and running with go can be done with "go build"
// then "./timeserver". This will show up on port 8080 by default.
// If the port is in use there will be an error. The port can also
// selected manually if a different port is desired
//
// The program uses seelog:
// https://github.com/cihub/seelog
// The implementation for this project was done by Ryan Warren
// This includes color coding the output message type in the
// command line and where to store the log files.
//
// The program has multiple command line options available to enter:
//
// "--port [port-number]" will change the port.
...
import (
	log "../../seelog-master/" //seelog work done by Ryan Warren
	"../counter"
	"bytes"
	"flag"
	"fmt"
...
//
// used to figure out the more complex template for this page:
// http://golangtutorials.blogspot.com/2011/06/go-templates.html
